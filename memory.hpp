...
namespace base::memory
{
    ...
    {
        if (syscallInitialized || PID == 0) return;
        ...
        if (!syscallReadStubPtr || !syscallWriteStubPtr) return;
    ...
    __forceinline T read(uint64_t address) noexcept
    {
        ...
        T buffer{};
        SIZE_T bytesRead{};
        if (handleProcess && NtReadVirtualMemory)
            NtReadVirtualMemory(handleProcess, reinterpret_cast<PVOID>(address), &buffer, sizeof(T), &bytesRead);
        return buffer;
    }
    ...
    __forceinline T syscall_read(uint64_t address) noexcept
    {
        ...
        if (handleProcess && syscall_NtReadVirtualMemory)
            syscall_NtReadVirtualMemory(handleProcess, reinterpret_cast<PVOID>(address), &buffer, sizeof(T), reinterpret_cast<PULONG>(&bytesRead));
        return buffer;
    }
    ...
    __forceinline T zw_read(uint64_t address) noexcept
    {
        ...
        if (handleProcess && ZwReadVirtualMemory)
            ZwReadVirtualMemory(handleProcess, reinterpret_cast<PVOID>(address), &buffer, sizeof(T), &bytesRead);
        return buffer;
    }
    ...
    {
        EnsureSyscallInit();
        if (!handleProcess || !NtWriteVirtualMemory) return false;

        SIZE_T bytesWritten{};
        NTSTATUS status = NtWriteVirtualMemory(handleProcess, reinterpret_cast<PVOID>(address), const_cast<void*>(buffer), size, &bytesWritten);
    ...
    {
        EnsureSyscallInit();
        if (!handleProcess || !syscall_NtWriteVirtualMemory) return false;

        SIZE_T bytesWritten{};
        NTSTATUS status = syscall_NtWriteVirtualMemory(handleProcess, reinterpret_cast<PVOID>(address),
    ...
    {
        EnsureSyscallInit();
        if (!handleProcess || !ZwWriteVirtualMemory) return false;

        SIZE_T bytesWritten{};
        NTSTATUS status = ZwWriteVirtualMemory(handleProcess, reinterpret_cast<PVOID>(address),
